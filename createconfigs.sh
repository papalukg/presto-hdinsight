...
cat > appConfig-default.json <<EOF
{
  ...
  "global": {
    ...
    "site.global.catalog": "{ $EXTRA_CONNECTORS 'hive': ['connector.name=hive-hadoop2','hive.metastore.uri=$metastore', 'hive.config.resources=/etc/hadoop/conf/wasb-site.xml'], 'tpch': ['connector.name=tpch']}",
